...
- 涉及管理员能力时，前端隐藏入口不能代替后端权限校验，后端必须强制校验 `role == "admin"`
- 系统默认知识库属于全局可读资源，不得当作某个普通用户的私有文档处理
...
- 当前用户类型必须包含 `role` 和 `is_active`，管理员入口只能对 `role == "admin"` 的用户展示
...
- 管理员接口必须集中在独立 admin 路由或清晰的 admin 模块中，禁止散落在普通用户接口里
- 默认管理员账号必须为 `admin/admin`，密码仍需使用项目统一哈希方式存储，禁止明文入库
...
- `backend/Default-know-base.md` 必须作为系统默认知识库导入
- 系统默认知识库必须标记为 `is_system = true`、`is_deletable = false`
- 系统默认知识库对所有登录用户可见、可查看详情、可参与检索
- 系统默认知识库禁止普通用户和管理员删除，删除接口必须返回明确 403
- Markdown 默认知识库切片必须走语义切片：标题、段落、列表、句子边界优先，长度切分只做兜底
- 默认知识库内容变更时，应通过稳定来源标识和内容哈希判断是否需要重建切片
...
- 当前用户可访问的知识范围包括：当前用户私有文档 + 系统默认知识库
- 检索必须支持混合检索：向量语义检索 + 关键词检索
- Embedding 或向量检索不可用时，关键词检索必须作为兜底，不能直接让默认知识库失效
- rerank 必须与召回解耦，优先融合语义分、关键词分、短语命中、标题命中、切片位置等稳定特征
- 返回给前端的引用分数应代表 rerank 后的综合相关度，而不是伪造的模型置信度
...
- 管理员可以进行知识库管理：
  ...
  - 同步系统默认知识库
  - 删除非系统知识库文档
- 管理员不能删除系统默认知识库，也不能删除内置 admin 账号或当前登录管理员账号
- 禁用用户后必须阻止其继续登录；已签发 token 的请求也应通过当前用户状态校验拦截
- 普通用户访问 admin 接口必须返回 403
